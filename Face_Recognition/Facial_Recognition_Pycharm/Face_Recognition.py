...
# Function to load the first 60 images from cifar 10 data set and use these as the non face images
...
# Function to load about 60 face images from Face_Images dir and format them
def load_face_images(dir_name):
    ...
    for image_name in glob.glob(dir_name + '/*'):
        ...
        output_image_data = np.transpose(image_data, (2, 0, 1))  # (32, 32, 3) -> (3, 32, 32)
...
def create_model():
    ...
    model.add(Flatten())
    model.add(Dense(521, activation='relu', kernel_constraint=maxnorm(3)))
    model.add(Dropout(0.5))
    model.add(Dense(2, activation='softmax'))
    return model


def train_and_test_model(model, train_images, train_labels):
    epochs = 50
    l_rate = 0.01
    decay = l_rate / epochs
    optimiizer = SGD(lr=l_rate, momentum=0.9, decay=decay, nesterov=False)
    model.compile(loss='categorical_crossentropy', optimiizer=optimiizer, metrics=['accuracy'])
    print(model.summary)
...
def predict_image(model, test_images, test_labels):
    for i in range(len(test_labels)):
        ...
        print('Predicted label: {}, actual label: {}'.format(model.predict(test_images), test_labels[i]))
...
def run():
    ...
    model = create_model()
    model = train_and_test_model(model, train_images, train_labels)
